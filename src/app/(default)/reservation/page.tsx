...
export default function ReservationPage({}: Props) {
    ...
    if (socket) {
      function handleBookSuccess(message: string, { tableId }: { tableId: string }) {
        if (message === 'Book table successfully') {
          ...
          const newTable = tableData.map((value) =>
            value._id !== tableId ? value : ({ ...value, tableStatus: 'Occupied' } as Table),
          );
          setTableId(null);
          setIsOpen(false);
      ...
      function handleCancelSuccess(message: string, { tableId }: { tableId: string }) {
        if (message === 'A table has been canceled') {
          ...
          const newTable = tableData.map((value) =>
            value._id !== tableId ? value : ({ ...value, tableStatus: 'Available' } as Table),
          );
          setTableData(newTable);
        }
      }
  ...
  const handleSubmit = () => {
    if (socket) {
      ...
      const bookingTime = `${dayjs(date).format('YYYY-MM-DD')} ${dayjs(time).format('HH:mm:ss')}`;
  ...
  const handleTableClick = (data: Table) => {
    ...
    setTableData(newTableData.map((value) => (value._id !== tableId ? value : { ...value, tableStatus: 'Available' })));
    setTableId(data._id);
    setDate(null);
    setTime(null);
    setCount('');
