...
export const getProductByNamePrice = (keyword: string, minPrice: number | null, maxPrice: number | null) => {
  return new Promise<Product[]>(async (resolve, reject) => {
    try {
      const res = await server(`/dishes/search?minPrice=${minPrice}&maxPrice=${maxPrice}&keyword=${keyword}`, {
        cache: 'no-store',
      });
      const products: Product[] = res.data as Product[];
      resolve(products);
...
export const filterDish = (minPrice: number, maxPrice: number, menuID: string, page: number, perPage: number) => {
  return new Promise<Product[]>(async (resolve, reject) => {
    try {
