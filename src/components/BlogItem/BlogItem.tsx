...
export default function BlogItem({
  ...
  return (
    <div className="flex flex-row gap-4 border-b border-black pb-2">
      <img className="w-40 object-contain" src={imgSrc || 'https://placehold.co/400x300'} alt={header} />
      <div className="flex flex-col">
        <Link href={`/blog/${slug}`} target="_blank">
          <h2 className="text-xl font-bold hover:underline">{header}</h2>
        </Link>
        <p className="text-xs font-light text-gray-400">{id}</p>
        <p className="text-sm text-gray-600">{dayjs(createdAt).format('hh:mm DD/MM/YYYY')}</p>
