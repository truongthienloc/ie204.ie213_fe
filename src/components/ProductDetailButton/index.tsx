...
function ProductDetailButtons({ product }: { product: Product }) {
  ...
  const handleAddProductToCart = async () => {
    ...
    const existingProduct: CartProduct | undefined = cartList.find((item) => item._id === product?._id);
  ...
  return (
    <>
      <div className={styles['btn__container']}>
        <button className={`${styles['detail__btn']} ${styles.primary}`} onClick={handleAddProductToCart}>
          Thêm vào giỏ hàng
        </button>
