...
export default function AdminSidebar() {
  ...
  return (
    <div className="mb-20 flex h-full w-[300px] flex-col gap-10 bg-[#f8f8f8] pl-8 pr-4 pt-8">
      ...
      <div className="flex flex-col">
        {menu.map((item) => {
          return (
            ...
            >
              <img className="aspect-square w-[25px]" src={item.src} alt={item.label} />
              {item.label}
            </Link>
          );
        })}
      ...
      <button
        className={'flex gap-4 rounded-sm px-[5px] py-[10px] text-lg font-normal text-second hover:text-primary'}
