...
export default function EditDishModal({ isOpen, dishInfo, onClose, onSubmit, menu }: Props) {
  ...
  const handleDeleteAvailableImage = (id: string) => {
    dishInfo.handleDeleteImage(id);
  };
        ...
        <div className="relative my-6">
          ...
          <div className="relative flex w-full flex-col rounded-lg border-0 bg-white outline-none focus:outline-none">
            ...
            <div className="flex  justify-center border-b border-solid p-5">
              <h3 className="text-2xl font-medium text-primary ">Sửa món ăn</h3>
            </div>
            {/*body*/}
            <div className="mt-4 flex flex-col justify-center gap-7 px-7   ">
              ...
              <div className="flex items-center gap-8 text-lg ">
                ...
                >
                  ...
                  {menu &&
                    menu.map((value) => (
                      <option key={value._id} id={value._id} value={value._id}>
                        {value.menuName}
                      </option>
                    ))}
                </select>
              ...
              <div className="flex items-center gap-8 text-lg ">
                ...
                <div className="grid grid-flow-row grid-cols-2 flex-wrap items-center gap-4">
                  {dishInfo.imgs.length > 0 &&
                    dishInfo.imgs.map((image) => (
                      <ImageItem key={image.id} image={image} onDelete={() => handleDeleteAvailableImage(image.id)} />
                    ))}
                  {images.length > 0 &&
                    images.map((image) => (
                      <ImageItem key={image.id} image={image} onDelete={() => handleDeleteImage(image.id)} />
                    ))}
                  <button
              ...
              <div className="flex items-center gap-8 text-lg ">
                ...
                <textarea
                  ...
                  // cols=''
                  rows={5}
                  className="h-[80px] w-[300px] rounded-lg border-2 border-primary px-3 focus:outline-none"
