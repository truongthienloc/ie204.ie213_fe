...
export default function useDiscount() {
  ...
  const handleChangeDiscountName = (e: React.ChangeEvent<HTMLInputElement>) => setDiscountName(e.target.value);
  const handleChangeDescription = (e: React.ChangeEvent<HTMLTextAreaElement>) => setDescription(e.target.value);
  const handleChangePercent = (e: React.ChangeEvent<HTMLInputElement>) => setPercent(e.target.value);
